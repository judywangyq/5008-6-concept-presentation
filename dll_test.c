...
int unitTest7(int status){
    ...
    char nameComp[15] = "Kuzmenko";
	//printf("name3 is %s\n", test->tail->previous->previous->name);
    if(0==strcmp(forward->name, nameComp)){
       passed = 1;
    }else{
    	passed = 0;
...
int unitTest8(int status){
    ...
    team_push_front(test, 65, player2);
    team_push_front(test, 35, player3);
    int testNum = team_list_get(test,2);	
    printf("num is %d\n", testNum);
...
// Tests push_front/back/, inserts at position 2, 
// check for invalid pos input for get(). should return 0
// tests the return value
int unitTest11(int status){
    ...
    team_push_back(test, 65, player2);
    team_push_front(test, 35, player3);
    team_insert(test, 2, 23, player4);
    
	if(0==team_list_get(test, -2)){
       passed = 1;
    }else{
    	passed = 0;
    }
...
int unitTest12(int status){
	...
	if(0==team_insert(test, -2, 23, player4)){
       passed = 1;
    }else{
    	passed = 0;
    }
